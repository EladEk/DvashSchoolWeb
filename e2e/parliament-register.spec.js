...
test.describe('Parliament register', () => {
  ...
  test('negative: short password shows error', async ({ page }) => {
    ...
    await page.locator('form').locator('button[type="submit"]').click()
    await expect(page.locator('.parliament-error')).toContainText(/4|קצר|short|תווים/i)
  })
  ...
  test('register form submit does not crash (may show success or connection/username error)', async ({ page }) => {
    ...
    await expect(page.locator('.parliament-login-page')).toBeVisible()
    const url = page.url()
    const hasError = await page.locator('.parliament-error').isVisible()
    const hasRedirect = url.includes('/parliament') && !url.includes('/parliament/login')
